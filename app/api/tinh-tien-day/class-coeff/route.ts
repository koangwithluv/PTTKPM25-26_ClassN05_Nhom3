...
export async function POST(req: NextRequest) {
  try {
    const { classType, coeff, description } = await req.json()
    const [result]: any = await db.query(
      'INSERT INTO ClassCoeff (classType, coeff, description) VALUES (?, ?, ?)',
      [classType, coeff, description]
    )
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const { classType, coeff, description } = await req.json()
    await db.query(
      'UPDATE ClassCoeff SET classType=?, coeff=?, description=? WHERE id=?',
      [classType, coeff, description, id]
    )
